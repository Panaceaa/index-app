...
function getApiUrl(query: string, chainId: number): string {
  ...
  if (networkKey.includes('mainnet')) {
    return `https://api.0x.org/${quotePath}?${query}&affiliateAddress=${ZeroExAffiliateAddress}`;
  }
  else {
    return `https://polygon.api.0x.org/${quotePath}?${query}&affiliateAddress=${ZeroExAffiliateAddress}`;
  }
